...
const BehavioralMockInterview = () => {
  ...
  const sessionType = 'MOCK_INTERVIEW_BEHAVIORAL';
  ...
  const behavioralData: ISinglesSession[] = data?.data || [];
  ...
  const handleViewSession = (sessionId: string) => {
    router.push(`/dashboard/mock-test/behavioral/${sessionId}`);
  };
  ...
  const handleFileSelect = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        router.push(`/dashboard/mock-test/behavioral/practice/${newSessionId}`);
      }
    } catch (error) {
      ...
      <div className='flex justify-between items-center'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Practice and review your behavioral interview skills.
          </p>
        </div>
        ...
        >
          ...
          {isCreating ? 'Starting...' : 'Start Behavioral Interview'}
        </Button>
      </div>
        ...
        <CardContent>
          <div className='space-y-4'>
            <CustomTable
              ...
              actions={actions}
              loading={isLoading}
              emptyMessage='No behavioral interview sessions found. Start one to practice!'
            />
