...
const TechnicalMockInterview = () => {
  ...
  const sessionType = 'MOCK_INTERVIEW_TECHNICAL';
  ...
  const technicalData: ISinglesSession[] = data?.data || [];
  ...
  const handleViewSession = (sessionId: string) => {
    router.push(`/dashboard/mock-test/technical/${sessionId}`);
  };
  ...
  const handleFileSelect = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        router.push(`/dashboard/mock-test/technical/practice/${newSessionId}`);
      }
    } catch (error) {
      ...
      <div className='flex justify-between items-center'>
        <div>
          ...
          <p className='text-muted-foreground'>
            Practice and review your technical interview skills.
          </p>
        </div>
        ...
        >
          ...
          {isCreating ? 'Starting...' : 'Start Technical Interview'}
        </Button>
      </div>
        ...
        <CardContent>
          <div className='space-y-4'>
            <CustomTable
              ...
              actions={actions}
              loading={isLoading}
              emptyMessage='No technical interview sessions found. Start one to practice!'
            />
...
export default TechnicalMockInterview;
