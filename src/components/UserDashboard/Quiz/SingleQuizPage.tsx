...
const SingleQuizPage = ({ id }: { id: string }) => {
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-3">
                  ...
                  <CardDescription className="flex items-center gap-2 text-slate-600 dark:text-slate-400">
                    ...
                    Completed on{" "}
                    {new Date(quiz.endedAt || quiz.createdAt).toLocaleDateString("en-US", {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
              ...
              <div className="flex items-center justify-between p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                <div className="flex items-center space-x-6">
                  <div className="text-center">
                    <div
                      className={`text-4xl font-bold ${quiz.score ? getScoreColor(quiz.score) : "text-slate-500 dark:text-slate-400"}`}
                    >
                      {quiz.score ?? "—"}
                    </div>
                    <div className="text-sm text-slate-600 dark:text-slate-400 font-medium">Score</div>
                  </div>
                ...
                {quiz.score && (
                  <Badge className={`px-4 py-2 text-sm font-medium ${getScoreBadge(quiz.score).color} border-0`}>
            ...
            <Card className="border-0 shadow-lg bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm overflow-hidden">
              <CardHeader className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                <CardTitle className="flex items-center justify-between">
                  <span className="text-xl font-semibold text-slate-900 dark:text-slate-100">Question {index + 1}</span>
                  <Badge
                  ...
                  >
                    {answer.isCorrect ? <CheckCircle className="mr-2 h-4 w-4" /> : <XCircle className="mr-2 h-4 w-4" />}
                ...
                {answer.question.options && (
                  <div className="space-y-3">
                    {answer.question.options.map((option, i) => {
                      ...
                      let optionClass = "border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800/30"
                      ...
                      return (
                        ...
                        >
                          ...
                          <span
                            className={`text-slate-900 dark:text-slate-100 ${isCorrect ? "font-semibold" : "font-medium"}`}
                          >
                            {option}
                          </span>
                        </div>
                  ...
                  <div className="grid md:grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 text-slate-900 dark:text-slate-100">
                        {answer.selectedOption || answer.answerText || "Not answered"}
                      </span>
                    </div>
                    <div>
